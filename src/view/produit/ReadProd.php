...
<body>
	<div class="container">
          ...
          <div class="input-group-pretend"> 
            <form class="form-inline mr-auto w-100 navbar-search">
               <div class="input-group">
                  ...
                  <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#exampleModal">search Prod&emsp;<i class="fas fa-search fa-sm"></i>
                  </button></a>
                </div>
            </form>
          </div> 
     ...
     <div class="box">
		<div class="alert alert alert-primary" role="alert">
			<h4 class="text-primary text-center">Liste Prod</h4>
		</div>
  ...
  <tbody>
    ...
    while ($rows = mysqli_fetch_assoc($result)) {
      ?>
    <tr>
        <td class="align-middle"><?=$rows['ref']?></td>
        <td class="align-middle"><?=$rows['nom']?></td>
        <td class="align-middle"><?=$rows['qtStock']?></td>
        <td class="align-middle"><?=$rows['id']?></td>
        <td class="align-middle">
          <a href="UpdateProd.php?ref=<?=$rows['ref']?>" 
             class="btn btn-success">Update</a>
          <a href="http://localhost/mes_cours/liagegda/suivistock/src/model/produit/DeleteProdDB.php?ref=<?=$rows['ref']?>" 
